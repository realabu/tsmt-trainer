export function getDisplayRepetitionsLabel(input: {
  repetitionsLabel?: string | null;
  repetitionCount?: number | string | null;
  repetitionUnitCount?: number | string | null;
}) {
  ...
  const repetitionCount =
    typeof input.repetitionCount === "string"
      ? Number.parseInt(input.repetitionCount.trim(), 10)
      : input.repetitionCount ?? null;
  const repetitionUnitCount =
    typeof input.repetitionUnitCount === "string"
  ...
  const safeUnitCount =
    repetitionUnitCount && !Number.isNaN(repetitionUnitCount) ? repetitionUnitCount : null;
